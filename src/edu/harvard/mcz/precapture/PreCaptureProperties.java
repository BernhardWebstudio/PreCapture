...
public class PreCaptureProperties extends AbstractTableModel {
    ...
    /**
     * Key for properties file for whether the inventory tab should be shown in
     * the UI.
     */
     ...
     * filtering a filtering combo box (Taxon name combo box).
     */
    public static final String KEY_FILTER_LENGTH_THRESHOLD =
            "config.filterlengththreshold";
    /**
     * Value to use for QRCode error correction level in hint to Zxing in
     * encoding.
     */
    public static final String KEY_QRCODEECLEVEL = "config.qr_errorcorrection";
    /**
     * Key for properties file for wheter or not the edit panel should be shown
    ...
    /**
     * Key for properties file for rank of subspecies in trinomials, 'subspecies'
     * by default.
     */
    public static final String KEY_TRINOMIAL_SUBSP_RANK =
            "config.trinomialsubsprank";
    /**
     * When not using quadranomials, put subspecies and subspecies rank in
     * infraspecific name and rank.
     */
    public static final String KEY_PUT_SUBSPECIES_IN_INFRA =
    ...
    private void checkDefaults() {
        ...
        if (!properties.containsKey(KEY_PRINTDEFINITIONS)) {
            ...
            properties.setProperty(KEY_PRINTDEFINITIONS,
                    "resources/LabelDefinitions.xml");
        }
        if (!properties.containsKey(KEY_SELECTED_PRINT_DEFINITION)) {
            // name of selected print layout definition
            properties.setProperty(KEY_SELECTED_PRINT_DEFINITION,
                    "Multiple 1.5x3 slips on 8.5x11 Larger");
        }
        ...
        if (!properties.containsKey(KEY_EXTRAHUMANTEXT)) {
            ...
            properties.setProperty(KEY_EXTRAHUMANTEXT,
                    "Do not remove this slip from this folder.");
        }
    ...
    public void saveProperties() throws Exception {
        ...
        try {
            ...
            properties.store(propertiesStream, PreCaptureApp.NAME + " " +
                    PreCaptureApp.VERSION +
                    " User Properties");
            propertiesStream.close();
        } catch (Exception e) {
